      ...
      <section className="content-section">
        ...
        {owned.length ? (
          <div className="member-grid">
            {owned.map((program) => (
              <article className="member-card" key={program.slug}>
                <span className="access-pill">Access active</span>
                <h3>{program.title}</h3>
                <p>{program.outcome}</p>
                <ul>{program.lessons.map((lesson) => <li key={lesson}>{lesson}</li>)}</ul>
                {program.free ? (
                  <a className="button button-primary" href="/downloads/calmer-family-week.txt">Download guide</a>
                ) : (
                  <Link className="button button-primary" href={`/members/programs/${program.slug}`}>Open program</Link>
                )}
              </article>
            ))}
          </div>
          ...
          <div className="empty-state">
            ...
            <p>Choose a program and it will appear here after checkout.</p>
            <Link className="button button-primary" href="/programs">Browse programs</Link>
          </div>
        )}
      </section>
      {discover.length > 0 && (
